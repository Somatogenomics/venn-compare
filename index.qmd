...
- **Live per-set item counts in the sidebar:** Each set name now includes a small badge (for example, “15 items”) that updates immediately as you paste or edit entries. Counts reflect your case/whitespace options and de-duplication.
- **Full region counts on the diagram (up to 4 sets):**
  - **2 sets:** A only, A ∩ B, B only (3 regions)
  - **3 sets:** all 7 mutually exclusive regions
  - **4 sets:** all 15 mutually exclusive regions (non-zero regions are labeled to keep the plot readable)
...
observe({
  ...
  for (i in 1:input$n_sets) {
    local({
      ...
      output[[paste0("count_", idx)]] <- renderText({ paste0(cnt, " items") })
    })
  }
})
...
output$main_plot <- renderPlot({
  ...
  if (n <= 4) {
    ...
    if (n == 2) {
      ...
      # Region positions for 2 sets: A only, A&B, B only
      regionpos <- list(c(-1.8,0), c(0,0), c(1.8,0))
    } else if (n == 3) {
      centers <- list(c(-1,0.6), c(1,0.6), c(0,-1)); r <- 1.6
      lblpos <- list(c(-2.5,2.2), c(2.5,2.2), c(0,-3))
      # Region positions for 3 sets
      regionpos <- list(
        c(-1.6,0.9),   # A only
        c(1.6,0.9),    # B only
        c(0,-1.7),     # C only
      ...
      # Region positions for 4 sets (approximate)
      regionpos <- list(
        c(-1.8,1.2),   # A only
        c(1.8,1.2),    # B only
        c(-1.3,-1.5),  # C only
    ...
    # Calculate and display counts for each region
    ints <- compute_intersections(sets)
    
    if (n == 2) {
      A <- sets[[1]]; B <- sets[[2]]
      counts <- c(
        length(setdiff(A, B)),           # A only
        length(intersect(A, B)),         # A&B
        length(setdiff(B, A))            # B only
      )
      for (i in 1:3) {
        text(regionpos[[i]][1], regionpos[[i]][2], counts[i], cex = 1.3, font = 2)
      }
    } else if (n == 3) {
      ...
      Aonly <- setdiff(setdiff(A, B), C)
      Bonly <- setdiff(setdiff(B, A), C)
      Conly <- setdiff(setdiff(C, A), B)
      
      counts <- c(length(Aonly), length(Bonly), length(Conly),
                  length(AB), length(AC), length(BC), length(ABC))
      for (i in 1:7) {
        text(regionpos[[i]][1], regionpos[[i]][2], counts[i], cex = 1.1, font = 2)
      }
    } else if (n == 4) {
      A <- sets[[1]]; B <- sets[[2]]; C <- sets[[3]]; D <- sets[[4]]
      
      # All 15 regions for 4 sets
      ABCD <- Reduce(intersect, sets)
      ...
      counts <- c(length(Aonly), length(Bonly), length(Conly), length(Donly),
                  length(AB), length(AC), length(AD), length(BC), length(BD), length(CD),
                  length(ABC), length(ABD), length(ACD), length(BCD), length(ABCD))
      for (i in 1:15) {
        if (counts[i] > 0) {
          text(regionpos[[i]][1], regionpos[[i]][2], counts[i], cex = 0.9, font = 2)
        }
      }
    }
